...
export class IngresarSolicitudComponent implements OnInit {
  ...
  guardar() {
    ...
    const solicitud = this.crearSolicitudModel();
  ...
  colombiaSeleccionado() {
    ...
    let paisSeleccionado = this.registerForm.value.PaisDestino.text;
  ...
  colombiaSeleccionadoExpedicionIntermediario() {
    ...
    let paisSeleccionado = this.registerForm.value.paisExpedicionIntermediario.text;
    if (paisSeleccionado == 'COLOMBIA') {
      return true;
    }
    else {
